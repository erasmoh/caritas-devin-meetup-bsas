...
function WallImageCounter({ count }: { count: number }) {
  return (
    <div className="pointer-events-none absolute left-5 top-36 z-50 bg-[#0F131C]/70 px-4 py-3 font-mono text-[#FA5050] shadow-[6px_6px_0_#0F131C] backdrop-blur-sm sm:left-8 sm:top-48 sm:px-5 sm:py-4">
      <div className="text-4xl font-black leading-none tracking-[0.08em] sm:text-6xl">
        {String(count).padStart(3, "0")}
      </div>
      <div className="mt-1 text-[10px] font-black uppercase tracking-[0.18em] text-red-100 sm:text-xs">
        imágenes en el muro
      </div>
    </div>
  );
...
export default function WallRealtime({ configured, initialImages, supabaseConfig }: WallRealtimeProps) {
  ...
  if (!configured) {
    return (
      <div className="relative z-10 flex min-h-dvh items-center justify-center p-6 text-center text-red-100">
      ...
      <>
        ...
        <div className="relative z-10 flex min-h-dvh items-center justify-center p-8 text-center">
          <div className="max-w-md space-y-3">
            <p className="text-3xl font-black text-[#FA5050]">Aún no hay cards en el muro</p>
      ...
      <section className="absolute inset-0">
        {images.map((image, index) => (
          <article
            ...
            className="absolute w-[38vw] min-w-[132px] max-w-[240px] overflow-hidden rounded-[1.15rem] bg-[#0F131C] shadow-[0_24px_70px_rgba(0,0,0,0.6)] transition duration-300 hover:z-50 hover:scale-110 sm:w-[24vw] md:w-[220px]"
          >
